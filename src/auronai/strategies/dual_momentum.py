...
class DualMomentumParams(StrategyParams):
    ...
    MODIFIED FOR CASH MANAGEMENT:
        top_n: 4 (instead of 5) to maintain 20% cash reserve
        risk_budget: 0.80 (80% invested, 20% cash)
    """
    lookback_period: int = 252  # 12 months (Antonacci standard)
    top_n: int = 4  # Hold top 4 assets (20% each = 80% invested)
...
class DualMomentumStrategy(BaseStrategy):
    ...
    # Flag to tell backtest_runner we need full historical data
    ...
    def _calculate_momentum(
        ...
        Args:
            features: DataFrame with OHLCV data (MultiIndex: symbol, date)
            current_date: Current simulation date
        ...
        # Expect MultiIndex (symbol, date) from backtest_runner
        if not isinstance(features.index, pd.MultiIndex):
            logger.warning(f"Expected MultiIndex but got {type(features.index)}")
            return momentum_scores
        
        level_names = features.index.names
        
        # Determine format
        if level_names[0] == 'symbol' and level_names[1] == 'date':
            symbols = features.index.get_level_values(0).unique()
        elif level_names[0] == 'date' and level_names[1] == 'symbol':
            symbols = features.index.get_level_values(1).unique()
        else:
            logger.warning(f"Unknown MultiIndex format: {level_names}")
            return momentum_scores
        
        for symbol in symbols:
            try:
                # Extract data for this symbol
                if level_names[0] == 'symbol':
                    symbol_data = features.xs(symbol, level=0).copy()
                else:
                ...
                if current_idx < self.params.lookback_period:
                    logger.debug(
                        f"{symbol}: insufficient history ({current_idx} < {self.params.lookback_period})"
                    )
                    continue
                
                # Get current and past prices (handle both 'close' and 'Close')
                close_col = 'close' if 'close' in symbol_data.columns else 'Close'
                current_price = symbol_data.iloc[current_idx][close_col]
                past_price = symbol_data.iloc[current_idx - self.params.lookback_period][close_col]
                ...
                if not np.isfinite(momentum):
                    logger.warning(f"{symbol}: non-finite momentum {momentum}")
                    continue
