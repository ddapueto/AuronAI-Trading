"""Swing Strategy MULTI-ASSET V2 - Inter-Sector Rotation.

Fase 2 de expansión multi-activo:
- Mantiene baseline strategy (TP 5%, 7 días, NO SL)
- Agrega sectores NO-TECH para diversificación completa
- Rotación inter-sectorial (tech, energía, finanzas, healthcare, industriales, consumo, bonos)
- Reduce dependencia del Nasdaq

Universo expandido:
- 10 acciones tech individuales (baseline)
- 5 ETFs tech (Fase 1)
- 6 ETFs sectores no-tech (Fase 2 - NUEVO)
- 1 ETF bonos (Fase 2 - NUEVO)
"""
...
class SwingMultiAssetV2:
    """
    Estrategia swing MULTI-ASSET V2 - Inter-Sector Rotation.
    
    Expansión Fase 2:
    - Agrega sectores no-tech (energía, finanzas, healthcare, industriales, consumo)
    - Agrega bonos (TLT) para diversificación defensiva
    - Rotación completa entre sectores
    - Reduce correlación con Nasdaq
    """
    ...
    def __init__(
        ...
        """
        Inicializar estrategia swing multi-asset V2.
        
        Args:
            symbols: Lista de símbolos (acciones + ETFs tech + ETFs sectores + bonos)
            benchmark: Símbolo de benchmark (default: 'QQQ')
            initial_capital: Capital inicial en USD
            base_risk_budget: Risk budget normal (default: 0.20)
            defensive_risk_budget: Risk budget defensivo (default: 0.05)
            ...
            dd_threshold_1: Drawdown threshold 1 (default: 0.05)
            dd_threshold_2: Drawdown threshold 2 (default: 0.08)
            dd_threshold_3: Drawdown threshold 3 (default: 0.10)
        ...
        logger.info(
            f"SwingMultiAssetV2 initialized: {len(symbols)} symbols "
            f"(stocks + tech ETFs + sector ETFs + bonds), capital=${initial_capital}"
        )
    ...
    def run_backtest(
        ...
        logger.info(
            f"Starting backtest (MULTI-ASSET V2 - Inter-Sector): {start_date} to {end_date}, "
